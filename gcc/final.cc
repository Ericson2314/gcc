	...
	switch (c)
	  ...
	  case 'U':
	    ...
	    /* Uppercase letters are reserved for general use by asm_fprintf
	       ...
	       Was inside `#ifdef ASM_FPRINTF_EXTENSIONS' with the macro
	       expansion below it.  This is a SHARED translation unit, so both
	       were the PRIMARY's -- see target-asmfprintf.h.  Unconditional
	       now, which changes nothing for i386 (it defines the macro, so
	       these labels were already present) and gives the other
	       forty-seven back ends the same reservation rather than sending
	       an uppercase letter to `gcc_unreachable ()' or, worse, to
	       another back end's case label.  */
	  ...
	  default:
	    /* The SELECTED back end's extension letters, not the primary's.
	       A back end that defines no `ASM_FPRINTF_EXTENSIONS' answers
	       false and lands on the same `gcc_unreachable ()' upstream gives
	       it; that is its own answer, not a floor.  */
	    if (!mt_asm_fprintf_extension (file, &argptr, c))
	      gcc_unreachable ();
	    break;
	  }
	break;
