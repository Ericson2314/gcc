...
/* THIS FILE IS COMPILED ONCE PER CONFIGURED BACK END, against that back end's
   own headers, as target-regstack-<cpu>.o.  See target-regstack.h for why,
   and for the list of symbols shared code still needs when this base has no
   register stack.  It is the body of what used to be reg-stack.cc; the pass
   objects that name it stayed behind there, shared.  */
...
/* The name of the table this translation unit defines.  Supplied by the
   per-base rule in gen-multi-target-md.awk as
   -DTARGETM_REGSTACK_SYMBOL=targetm_regstack_<cpu>; refuse rather than guess,
   because a default here would give every base the same symbol and the
   linker would keep one of them.  */
#ifndef TARGETM_REGSTACK_SYMBOL
#error "TARGETM_REGSTACK_SYMBOL is not defined; target-regstack.cc must be compiled per back end"
#endif
#ifndef MULTI_TARGET_TARGETM_BASE
#error "MULTI_TARGET_TARGETM_BASE is not defined; target-regstack.cc must be compiled per back end"
#endif

#define MT_STRINGIFY_1(X) #X
#define MT_STRINGIFY(X) MT_STRINGIFY_1 (X)
...
/* Return true if INSN mentions stacked registers, else return zero.
   ...
   STATIC, and renamed from `stack_regs_mentioned'.  This translation unit is
   compiled once per back end, so the old external name would be defined as
   many times as there are back ends with a register stack, and rtl.h declares
   that name `extern' -- a `static' redeclaration of it is ill-formed.  The one
   external definition shared code links against is the wrapper in
   target-regstack-select.cc, which reaches this through the table.  */

static bool
mt_stack_regs_mentioned (const_rtx insn)
{
...
subst_stack_regs (rtx_insn *insn, stack_ptr regstack)
{
  ...
  if (mt_stack_regs_mentioned (insn))
    {
...
convert_regs_1 (basic_block block)
{
  ...
  do
      ...
      if (DEBUG_BIND_INSN_P (insn))
	{
      ...
      else if (mt_stack_regs_mentioned (insn)
	       || CALL_P (insn))
	{
	  if (dump_file)
	    {
...
reg_to_stack (void)
{
  ...
  /* Allocate a cache for mt_stack_regs_mentioned.  */
  max_uid = get_max_uid ();
...
/* The table.  Two arms, both of them THIS back end's own answer to
   `#ifdef STACK_REGS' rather than the primary's.

   The redundant-looking `extern' declaration before each definition is
   load-bearing and target-cumargs.cc carries the same one: a namespace-scope
   `const' object has INTERNAL linkage in C++, so without it this definition
   is invisible to the linker and the registry in target-regstack-select.cc
   fails with `undefined reference to targetm_regstack_i386'.  Measured, not
   remembered -- that is exactly how this file first failed to link.  */

#ifdef STACK_REGS
extern const struct target_regstack_desc TARGETM_REGSTACK_SYMBOL;
const struct target_regstack_desc TARGETM_REGSTACK_SYMBOL = {
  MT_STRINGIFY (MULTI_TARGET_TARGETM_BASE),
  true,
  mt_stack_regs_mentioned,
  reg_to_stack
};
#else
/* A back end with no register stack.  The hooks are null rather than pointing
   at a shared "return false": the wrapper in target-regstack-select.cc has to
   handle a null anyway, because `targetm_regstack' is null before a target is
   selected, and two ways of spelling the same nothing is one too many.  */
extern const struct target_regstack_desc TARGETM_REGSTACK_SYMBOL;
const struct target_regstack_desc TARGETM_REGSTACK_SYMBOL = {
  MT_STRINGIFY (MULTI_TARGET_TARGETM_BASE),
  false,
