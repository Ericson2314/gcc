...
/* THE MULTI-TARGET CONVERSION LAYER USED TO BE THE REST OF THIS FILE.

   It is now `multi-target-macros.h', and this include is one of its two call
   sites -- the one that preserves today's ordering exactly: it is reached at
   the end of every `tm.h' / `tm-<base>.h', after every back-end header and
   after every fallback above.  The other call site is the four channel
   headers (`backend.h', `target.h', `cp/cp-tree.h',
   `m2/gm2-gcc/gcc-consolidation.h'), immediately after their `#include
   "tm.h"'; that one is the route that survives the deletion of `tm.h'.

   The split exists because `defaults.h' has no source-level includers at all,
   so before it the conversion layer was reachable ONLY through the header
   this branch exists to delete.  See the comment at the top of
