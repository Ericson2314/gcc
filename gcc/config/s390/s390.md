...
;; Processor type.  This attribute must exactly match the s390_processor_type
...
(define_split
  ...
  "reload_completed
   ...
   && known_gt (GET_MODE_SIZE (GET_MODE (operands[0])), 0)"
  [(parallel
    [(set (match_dup 0) (match_dup 1))
     (use (match_dup 2))])]
{
  operands[2] = GEN_INT (GET_MODE_SIZE (GET_MODE (operands[0])).to_constant ());
...
(define_insn "*execute_rl"
  ...
  "TARGET_Z10 && GET_MODE_CLASS (GET_MODE (operands[1])) == MODE_INT
   && known_le (GET_MODE_SIZE (GET_MODE (operands[1])), UNITS_PER_WORD)"
  "exrl\t%1,%3"
  [(set_attr "op_type" "RIL")
   (set_attr "type"    "cs")
   (set_attr "relative_long" "yes")])
...
(define_expand "atomic_load<mode>"
  ...
  ""
{
  if (known_lt (MEM_ALIGN (operands[1]), GET_MODE_BITSIZE (GET_MODE (operands[1]))))
    FAIL;
  ...
  ""
{
  ...
  if (known_lt (MEM_ALIGN (operands[0]), GET_MODE_BITSIZE (GET_MODE (operands[0]))))
    FAIL;
  ...
  ""
{
  ...
  unsigned int align = GET_MODE_BITSIZE (mode).to_constant ();
  s390_output_pool_entry (operands[0], mode, align);
  return "";
}
  [(set (attr "length")
        (symbol_ref "GET_MODE_SIZE (GET_MODE (PATTERN (insn))).to_constant ()"))])
