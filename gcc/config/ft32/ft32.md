...
(define_expand "prologue"
  ...
  ""
{
  /* MULTI-TARGET: the redundant local `extern void ft32_expand_prologue ();'
     that stood here is DELETED.  genemit wraps insn bodies in
     `namespace insn_ft32 { ... }', so a declaration written inside a body
     declares `insn_ft32::ft32_expand_prologue' -- a DIFFERENT function from
     the one `ft32.cc' defines at global scope, and the link failed with
     `undefined reference to insn_ft32::ft32_expand_prologue()'.
     `ft32-protos.h', reached through `tm_p-ft32.h', already declares it
     correctly at global scope.  */
...
(define_expand "epilogue"
  ...
  ""
{
  /* MULTI-TARGET: deleted for the same reason as in `prologue' above.  */
