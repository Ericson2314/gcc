...
# THE TARGET-AGNOSTIC HALF, INSTALLED ONCE.  See the note in the per-target loop
# for why these two are gcc's and why the directory is not `include-<cpu>'.
#
# ONCE, NOT PER TARGET, AND THE COUNT IS THE POINT: these files were previously
# copied N times, one per configured triple, into directories named after
# triples.  Nothing about either file depends on the triple, so N-1 of those
# copies were pure duplication AND the placement asserted a dependence that does
# not exist.
gen_dir=$dest_root/gen-headers
mkdir -p "$gen_dir" || fail "cannot create $gen_dir"
for f in auto-host.h version.h; do
  test -f "$builddir/$f" || fail "$builddir/$f is absent, and it is not
  optional: auto-host.h reaches libgcc through tconfig.h and is opened by 197 of
  452 translation units in a standalone libgcc build, and version.h is opened by
  the 28 _gcov_* objects through gcc/gcov-io.h.  Removing version.h on the
  grounds that no file under libgcc/ includes it was measured WRONG and reverted
  (#268 arm A): _gcov_merge_add.o is 7456 bytes with it and does not compile
  without it."
  cp "$builddir/$f" "$gen_dir/$f.tmp" && mv "$gen_dir/$f.tmp" "$gen_dir/$f"
done
echo "install-target-headers: auto-host.h version.h -> $final_root/gen-headers (once, not per target)"
...
for t in "$@"; do
  ...
  # THE THREE FILES THAT USED TO BE COPIED HERE HAVE OWNERS, AND NONE OF THE
  # OWNERS IS "THIS TARGET".  `tconfig.h', `auto-host.h' and `version.h' were
  # copied into EVERY target's directory, N identical copies of files that are
  # not per-target at all.  The user's ruling is that each component's generated
  # headers live in that component's own include directory, so they split by
  # OWNER rather than by consumer:
  #
  #   auto-host.h  gcc's, and target-agnostic -> $dest_root/gen-headers/, once.
  #                It is gcc's HOST configuration -- a fact about the machine gcc
  #                RUNS on -- and it reaches libgcc through tconfig.h.
  #
  #   version.h    gcc's too, and it is worth saying WHY it is not libgcc's even
  #                though libgcc opens it: it arrives through `gcc/gcov-io.h:240',
  #                which `libgcc/libgcov.h:191' includes.  It is gcc's header
  #                that libgcc opens, not a libgcc header.  -> gen-headers/, once.
  #
  #   tconfig.h    LIBGCC's, and generated by libgcc's own build system rather
  #                than copied from here at all.  Two reasons, and the second is
  #                a live bug: (a) it is a libgcc input and nothing else opens
  #                it; (b) gcc generates it ONCE PER BUILD from the `--target''s
  #                xm_file and then copies it to every target, so a build whose
  #                i386 slot is mingw hands mingw's tconfig.h to aarch64.  It is
  #                NOT target-independent -- measured across seven triples, four
  #                differ, each by the one #include its config.gcc `case
  #                ${target}' arm selects (i386/xm-mingw32.h, i386/xm-djgpp.h,
  #                i386/xm-cygwin.h, vms/xm-vms.h against the empty default).
  #                See libgcc/configure.ac.
  #
  # WHY gen-headers/ AND NOT include-<cpu>/.  `auto-host.h' and `version.h' are
  # PLAIN-NAMED, and `$(libsubdir)/include-<base>' is on every USER compile's
  # include path: GCC_BASE_INCLUDE_DIR_PREFIX is "$(libsubdir)/include-"
  # (Makefile.in:5820) and cppdefault.cc:156 concatenates it with the selected
  # base.  Putting a file called `version.h' there would offer it to every
  # `#include "version.h"' in user code.  `gen-headers' does not begin with
  # `include-', so cpp never searches it; libgcc reaches it by explicit -I.
