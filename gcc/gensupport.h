...
/* GEN_HDR_SUFFIX, gen_target_ns (), gen_multi_target_p () and the namespace
   printers used to be written out here.  They moved to gen-target-ns.h/.cc so
   that build/genconstants -- the program that WRITES insn-constants.h -- can
   link the single authority for them WITHOUT linking gensupport.cc, which is
   compiled against rtl.h, hence tm.h, hence insn-constants.h.  See the
   comment at the top of gen-target-ns.cc for the cold-build failure that
   forced the split.  Included here so that every existing user continues to
   get them from gensupport.h alone.  */
