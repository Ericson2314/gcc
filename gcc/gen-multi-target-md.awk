...
function emit_triple(	key, hdrs, i, n, parts, ssh, ssdep) {
  ...
  #
  # `gtype-desc.h' IS A REAL PREREQUISITE AND ITS ABSENCE PRESENTED AS A
  # PARALLELISM BUG.  It is not reached through gen-target-specs.cc's own
  # text; it is reached through the tm.h chain of ONE back end:
  #
  #   gen-target-specs.cc:81 -> tm-amdgcn_unknown_amdhsa.h:41
  #     -> config/gcn/gcn.h:771 "hash-table.h" -> hash-table.h:247 "ggc.h"
  #     -> ggc.h:30 "gtype-desc.h"
  #
  # and `gtype-desc.h' is generated by `s-gtype'.  Undeclared, this rule is a
  # coin toss against whichever job happens to be running `s-gtype': it
  # succeeded on every serial build and on most parallel ones, and failed a
  # 47-base `-j8' build with `fatal error: gtype-desc.h: No such file or
  # directory' -- i.e. the loudest possible diagnostic attached to the least
  # informative cause.  Declared for EVERY key rather than for gcn, because
  # the property that decides it is what a back end's headers include, and no
  # rule here reads that.  A `-DGENERATOR_FILE' object legitimately reaching a
  # GC header is unusual, which is exactly why nobody looked for it.
  printf "build/gen-target-specs-%s.o : $(srcdir)/gen-target-specs.cc \\\n", key;
  printf "  tm-%s.h $(srcdir)/spec-names.h gtype-desc.h \\\n", key;
  printf "  $(BCONFIG_H) $(SYSTEM_H) $(CORETYPES_H)\n";
