...
struct target_regs {
  ...
  unsigned char x_hard_regno_nregs[MULTI_TARGET_UNION_FIRST_PSEUDO_REGISTER][MAX_MACHINE_MODE];
  ...
  /* For each hard register, the widest mode object that it can contain.
     ...
     register.  */
  machine_mode x_reg_raw_mode[MULTI_TARGET_UNION_FIRST_PSEUDO_REGISTER];
  ...
  char x_contains_reg_of_mode[MULTI_TARGET_UNION_N_REG_CLASSES][MAX_MACHINE_MODE];

  /* 1 if the corresponding class contains a register of the given mode
     which is not global and can therefore be allocated.  */
  char x_contains_allocatable_reg_of_mode[MULTI_TARGET_UNION_N_REG_CLASSES][MAX_MACHINE_MODE];
