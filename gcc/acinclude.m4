...
dnl --enable-initfini-array and the probe behind it are gone.  The probe ran a
dnl `case "${target}"' (a special ia64 AC_RUN_IFELSE, an in-tree GNU ld >= 2.22
dnl version test, Solaris-specific section syntax, and a glibc feature-test
dnl preprocessor check), and answered "no" for every cross compiler except
dnl aarch64*-linux-gnu.  .init_array/.fini_array availability is a property of
dnl the target object format, so HAVE_INITFINI_ARRAY_SUPPORT is now 1 in
dnl config/elfos.h with a 0 default in defaults.h.
