#!/bin/sh
# #157 (from t150-rename-gap.sh) -- IS `MULTI_TARGET_RENAME_NAMES' COMPLETE FOR N BASES?
#
...
#
# usage: t157-rename-gap.sh <builddir>
set -u
S=$(cd "$(dirname "$0")" && pwd)
B=${1:?build dir}
case "$B" in
...
for b in $bases; do
  sh "$S/eb-shell.sh" "cd $G && nm -C --defined-only mt-$b/*.o 2>/dev/null" \
    ...
    | sort -u > "$B/t157-syms-$b.txt"
  n=$(grep -c . "$B/t157-syms-$b.txt" || true)
...
cat "$B"/t157-syms-*.txt | sort | uniq -d > "$B/t157-collisions.txt"
nc=$(grep -c . "$B/t157-collisions.txt" || true)
echo "  $nc colliding names over $nb bases"
echo "  (pairwise, per base, so a name in 3 bases counts once here)"
head -60 "$B/t157-collisions.txt" | sed 's/^/    /'
echo
