...
# that exact combination walls in `insn-emit' (see t134-alloca-wall.sh), which
...
sh "$S/eb-shell.sh" "cd $B/gcc && nm --version | head -1" > "$B/t134-nm-ver" 2>&1
cat "$B/t134-nm-ver"
grep -qi 'GNU nm' "$B/t134-nm-ver" || { echo "FATAL: no nm"; exit 9; }
...
for base in i386 aarch64; do
  o="$B/gcc/target-cumargs-$base.o"
  sh "$S/eb-shell.sh" "nm -C --defined-only $o" > "$B/t134-nm-$base.txt" 2>&1
  n=$(wc -l < "$B/t134-nm-$base.txt")
  [ "$n" -gt 5 ] || { echo "FATAL: nm on $o gave $n lines"; exit 9; }
  c=$(awk -v f="$SYM" 'index ($0, f) { print }' "$B/t134-nm-$base.txt" | wc -l)
  echo "$base: nm lines=$n  $SYM definitions=$c"
  awk -v f="$SYM" 'index ($0, f) { print "   " $0 }' "$B/t134-nm-$base.txt"
done
...
for base in i386 aarch64; do
  o="$B/gcc/target-cumargs-$base.o"
  sh "$S/eb-shell.sh" "objdump -d -C $o" > "$B/t134-dis-$base.txt" 2>&1
  n=$(wc -l < "$B/t134-dis-$base.txt")
  [ "$n" -gt 100 ] || { echo "FATAL: objdump on $o gave $n lines"; exit 9; }
  awk -v f="$SYM" '
    index ($0, f) && index ($0, ">:") { on = 1 }
    on { print }
    on && /^$/ { exit }
  ' "$B/t134-dis-$base.txt" > "$B/t134-body-$base.txt"
  bl=$(wc -l < "$B/t134-body-$base.txt")
  echo "--- $base  ($bl lines)"
  [ "$bl" -gt 2 ] || { echo "FATAL: empty cut for $base -- refusing to score"; exit 9; }
  sed -n '1,40p' "$B/t134-body-$base.txt"
done

echo
echo "=== ARM 3: the selector is bound by shared objects ==="
sh "$S/eb-shell.sh" "cd $B/gcc && nm -uC *.o" > "$B/t134-undef.txt" 2>&1
u=$(wc -l < "$B/t134-undef.txt")
...
sh "$S/eb-shell.sh" "cd $B/gcc && nm -uC --print-file-name *.o" \
  > "$B/t134-undef-f.txt" 2>&1
awk 'index ($0, "mt_stack_dynamic_offset") { print "   " $0 }' "$B/t134-undef-f.txt"
echo "objects binding mt_stack_dynamic_offset: $(awk 'index ($0, "mt_stack_dynamic_offset")' "$B/t134-undef-f.txt" | wc -l)"

echo
echo "=== ARM 4: function.o must no longer bind anything from the old ladder ==="
awk 'index ($0, "function.o") && (index ($0, "ix86_reg_parm_stack_space") || index ($0, "ix86_function_type_abi")) { print "   " $0 }' "$B/t134-undef-f.txt"
