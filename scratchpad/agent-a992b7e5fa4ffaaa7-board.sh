...
#
# The load sampler runs for the whole board and writes loadsamples.txt.  Above
# ~25 fifteen-minute load a row is PROVISIONAL and must be labelled; that
# cannot be decided after the fact from a single reading at scoring time, which
# is what the last board had to do.
set -u
ID=agent-a992b7e5fa4ffaaa7
...
( while :; do printf '%s %s\n' "$(date '+%H:%M:%S')" "$(cat /proc/loadavg)"; sleep 300; done ) \
  > "$L/loadsamples.txt" 2>&1 &
SAMPLER=$!
trap 'kill $SAMPLER 2>/dev/null' 0

echo "=== board start $(date); load $(cat /proc/loadavg)" > "$L/board.log"
echo "MT_MAKEFLAGS=-j12 MT_COMPILE_ONLY=1" >> "$L/board.log"
