#!/bin/sh
# #165 -- build in a t165-conf.sh build dir.  $1 = build dir, $2 = make target.
# Derived from t157-build.sh.  Re-asserts the snapshot at EVERY build.
#
# The `.rc' STAMP is the part that matters and it is written ONLY after make
# returns: PRINCIPLES 4 records an agent that reported "13 errors -> 7" from
# two mid-build snapshots and withdrew both figures, because its non-vacuity
# arm tested existence and non-emptiness -- the two properties a truncated log
# has.  t165-score.sh REFUSES a log with no stamp.
set -u
S=$(cd "$(dirname "$0")" && pwd)
D=${1:?build dir}
T=${2:-cc1}
