...
G=$(cd "$S/../gcc" && pwd)
F=$G/defaults.h
MODE=${1:?usage: t134-inject.sh off|on}

case $MODE in
  off)
    [ -f "$S/t134-defaults.h.orig" ] && { echo "REFUSE: a saved original already exists"; exit 9; }
    cp "$F" "$S/t134-defaults.h.orig"
    sed -e 's|^#undef ACCUMULATE_OUTGOING_ARGS$|/* T132-INJECTED-OFF: #undef ACCUMULATE_OUTGOING_ARGS */|' \
        ...
        "$S/t134-defaults.h.orig" > "$F"
    ;;
  on)
    [ -f "$S/t134-defaults.h.orig" ] || { echo "REFUSE: no saved original"; exit 9; }
    cp "$S/t134-defaults.h.orig" "$F"
    rm -f "$S/t134-defaults.h.orig"
    ;;
  *) echo "usage: t134-inject.sh off|on"; exit 9;;
esac
