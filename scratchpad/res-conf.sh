...
#
# usage: mtr-conf.sh <builddir> <comma-separated-triples | file-of-triples>
set -e
S=$(cd "$(dirname "$0")" && pwd)
SRC=$(cd "$S/.." && pwd)
D=${1:?build dir}
...
# 47, not the 45 the pre-existing conf scripts assert: the register-probe
# DEPFILES rule names MULTI_TARGET_REG_PROBES twice.  An EXACT value is
# asserted, not a `>=', so that a tree missing the change fails here rather
# than building and reporting a green for a compiler that is not this one --
# the exactness is the invariant, not the number, so this is bumped when the
# tree changes and never relaxed.
WANT=${WANT_ANCHOR:-47}
