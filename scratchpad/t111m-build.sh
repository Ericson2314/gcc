#!/bin/sh
# Task #59 build.  Env: D = build dir (default /tmp/b111m), SUB = subdir to
...
SRC=/home/jcericson/src/gnu/gcc/.claude/worktrees/agent-a583ac0157ff44074
D=${D:-/tmp/b111m}
SUB=${SUB:-gcc}
...
if [ ! -f "$D/config.status" ]; then
  ...
  sh_run "cd $D && $SRC/configure --disable-werror \
      --enable-backends=x86_64-pc-linux-gnu,msp430-unknown-elf \
