...
# NOT `git diff --quiet': the srcdir here is a `git archive' SNAPSHOT with no
# .git at all, so git walks UP out of /tmp, finds no repository, and the
# assertion ERRORS -- which this script then read as "the sources changed
# under the build" and refused a correct tree.  A check that cannot run is not
# a check.  Assert the two properties the snapshot actually has instead, the
# same pair t189-conf.sh asserts: its SHA stamp exists, and it is read-only.
