...
for T in "$@"; do
  ...
  # THIS SCORER PRODUCED TAA-BOARD.md, AND ITS ERROR COLUMN COUNTED LINES.
  # One aborted `.exp' emits THREE `^ERROR: ' lines and GCC's parallel harness
  # repeats the block once per runtest slot, so the column SCALED WITH `-j'.
  # Ported verbatim from mtscore.sh, which is the live scorer; see its comment
  # for the measured decomposition (visium: 29 lines = 8 causes = 1 `.exp').
  ...
  # ERROR is a harness failure, not a test result, and must not be able to
  # certify a run that produced no test result at all.
  tot=$((p+f+xp+xf+u+ur))
  if [ "$tot" -eq 0 ]; then
