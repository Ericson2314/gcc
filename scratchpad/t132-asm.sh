...
nix-shell -I "nixpkgs=$NP" \
  ...
  --substituters 'https://cache.nixos.org/' --run "
    ...
    echo '--- assembling:'
    aarch64-unknown-linux-gnu-as -o $B/t132-add.o $IN
    echo '--- file type (must say aarch64):'
    aarch64-unknown-linux-gnu-readelf -h $B/t132-add.o | grep Machine
    echo '--- disassembly:'
    aarch64-unknown-linux-gnu-objdump -d $B/t132-add.o
    echo '--- unwind table, decoded from the assembled object:'
    aarch64-unknown-linux-gnu-readelf --debug-dump=frames $B/t132-add.o
  "
echo "rc=$?"
